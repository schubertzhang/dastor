...
public class ColumnFamilyStore implements ColumnFamilyStoreMBean
{
    ...
    public static ColumnFamilyStore createColumnFamilyStore(String table, String columnFamily) throws IOException
    {
        ...
        try
        {
            String mbeanName = "com.bigdata.dastor.db:type=ColumnFamilyStores,keyspace=" + table + ",columnfamily=" + columnFamily;
    ...
    public void forceBlockingFlushBinary() throws InterruptedException
    {
        ...
        submitFlush(binaryMemtable_.get()).await();        
    }
...
// TODO this actually isn't a good meature of pending tasks
    ...
    public void reset()
    {
        ...
        if (logger_.isInfoEnabled())
            logger_.info("Begin reseting CF: " +  kscfName);
        ...
        if (logger_.isInfoEnabled())
            logger_.info("Marking all live sstables compacted to be delete by GC, of CF: " + kscfName);        
        try
        {
            markCompacted(ssTables_.getSSTables());
        }
        catch (IOException e)
        {
            logger_.error("IOException marking all live sstables compacted to be delete by GC, of CF: "  
                    + kscfName + "\n" + ExceptionUtils.getFullStackTrace(e));
        }
        
        // flush memtable, a new sstable may be generated, and commitlog will be discarded.
        if (logger_.isInfoEnabled())
            logger_.info("Flushing memtable of CF: " + kscfName);
        try
        {
        ...
        {
            logger_.error("Exception when flushing memtable of CF: "   
                    + kscfName + "\n" + ExceptionUtils.getFullStackTrace(e));
        }
        ...
        if (logger_.isInfoEnabled())
            logger_.info("Marking the new flushed sstable as compacted to be delete by GC, of CF: " + kscfName);
        try
        {
            markCompacted(ssTables_.getSSTables());
        }
        ...
        if (logger_.isInfoEnabled())
            logger_.info("Finish reseting CF : " + kscfName);
