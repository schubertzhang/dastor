...
import com.bigdata.dastor.concurrent.*;
...
import com.bigdata.dastor.gms.*;
import com.bigdata.dastor.io.DeletionService;
import com.bigdata.dastor.io.IndexSummary;
import com.bigdata.dastor.io.util.FileUtils;
import com.bigdata.dastor.io.util.FileUtils.FSInfo;
import com.bigdata.dastor.locator.*;
import com.bigdata.dastor.net.*;
import com.bigdata.dastor.service.AntiEntropyService.TreeRequestVerbHandler;
import com.bigdata.dastor.streaming.*;
...
public class StorageService implements IEndPointStateChangeSubscriber, StorageServiceMBean
{
    ...
    // BIGDATA: for globally statistic storage load
    public final AtomicLong storageLiveSize = new AtomicLong();
    public final AtomicLong storageTotalSize = new AtomicLong();
    ...
    public synchronized void initServer() throws IOException, com.bigdata.dastor.config.ConfigurationException
    {
        if (initialized)
        {
            if (isClientMode)
        ...
        // BIGDATA: load saved CF meta from system table.
        logger_.info("Starting to load saved CFStatus");
        SystemTable.loadKscfMetaFromSystemTable();

        // BIGDATA: check and reset the unfinished reseting CFs.
        logger_.info("Checking the unfinished reseting CFs and reset them");
        Table.checkUnfinishedResetingCF();

        // BIGDATA: start CFC thread
        logger_.info("Starting CFC");
        Collector.initTask();
        
        // BIGDATA:
        logger_.info("Starting up membership protocol to construct cluster topo...");
    ...
    */
    ...
    {
        double bytes = 0;
        /* BIGDATA disable this code segment for fast get result
        ...
        */
        ...
        // BIGDATA use following line code for fast get result
        bytes = StorageService.instance.storageLiveSize.get();
        return bytes;
    }
    ...
    */
    ...
    {
        getValidTable(tableName).forceCleanup(columnFamilies);
    }

    public void forceTableCompaction(int minCount) throws IOException
    {
        for (Table table : Table.all())
            table.forceCompaction(minCount);
    }

    // BIGDATA:
    @Override
    public void forceTableCompaction(int minCount, String tableName, String... columnFamilies) throws IOException
    {
        getValidTable(tableName).forceCompaction(minCount, columnFamilies);
    }
    ...
    */
    ...
    // BIGDATA:
    // force reset a column family, all data will be deleted. risk!
    public void forceResetColumnFamily(String keyspace, String columnFamily)
    {
        try
        {
            Table.open(keyspace).resetColumnFamily(columnFamily);
        }
        catch (IOException e) 
        {
            logger_.error("IOException when force reset CF: " + keyspace+":"+columnFamily);
        }
    }
    ...
    public double getGrossLoad()
    {
        ...
        for (String tableName : DatabaseDescriptor.getTables())
        {
            ...
            {
                ColumnFamilyStore cfs = table.getColumnFamilyStore(cfName);
                bytes += cfs.getTotalDiskSpaceUsed();
            }
        }
        */
        
        // BIGDATA use following line code for fast get result
        bytes = StorageService.instance.storageTotalSize.get();
        return bytes;
    }

    // BIGDATA:
    @Override
    public String getGrossLoadString()
    {
        return FileUtils.stringifyFileSize(getGrossLoad());
    }

    // BIGDATA:
    @Override
    public DiskSpaceLoad getDiskSpaceLoad()
    {
        DiskSpaceLoad load = new DiskSpaceLoad();
        ...
        for (String tableName : DatabaseDescriptor.getTables())
        {
            ...
            {
                ColumnFamilyStore cfs = table.getColumnFamilyStore(cfName);
                load.net += cfs.getLiveDiskSpaceUsed();
                load.gross += cfs.getTotalDiskSpaceUsed();
            }
        }
        */
        
        // BIGDATA use following line code for fast get result
        load.net = StorageService.instance.storageLiveSize.get();
        load.gross = StorageService.instance.storageTotalSize.get();
    ...
    // BIGDATA:
    @Override
    public DiskSpaceLoad getTableDiskSpaceLoad(String tableName)
    {
        DiskSpaceLoad load = new DiskSpaceLoad();
        Table table = null;
        try
        ...
        {
            ColumnFamilyStore cfs = table.getColumnFamilyStore(cfName);
            load.net += cfs.getTotalDiskSpaceUsed();
            load.gross += cfs.getTotalDiskSpaceUsed();
        }
        return load;
    }
    ...
    // BIGDATA:
    @Override
    public Map<String, DiskSpaceLoad> getTablesDiskSpaceLoad()
    {
        Map<String, DiskSpaceLoad> loadMap = new HashMap<String, DiskSpaceLoad>();
        for (String tableName : DatabaseDescriptor.getTables())
        {
            loadMap.put(tableName, getTableDiskSpaceLoad(tableName));
        }
        return loadMap;
    }
    ...
    // BIGDATA:
    @Override
    public int getCommitLogSegmentCount()
    {
        return CommitLog.instance().getSegmentCount();
    }

    // BIGDATA:
    @Override
    public List<byte[]> getSchema() throws IOException 
    {
        List<byte[]> schemaList = new ArrayList<byte[]>();
        Set<String> tables = DatabaseDescriptor.getTables();
        
        for (String table : tables)
        {
            schemaList.add(KSMetaData.serialize(DatabaseDescriptor.getKSMetaData(table)));
        }
        return schemaList;
    }
    ...
    // BIGDATA:
    @Override
    public Map<String, FSInfo> getLogFSInfo() throws IOException, InterruptedException {
        String path = DatabaseDescriptor.getLogFileLocation();
        Map<String, FSInfo> map = new HashMap<String, FSInfo>();
        map.put(path, FileUtils.getFSInfo(path));
        return map;
    }

    // BIGDATA:
    @Override
    public Map<String, FSInfo> getStorageFSInfo() throws IOException, InterruptedException {
