...
import static com.bigdata.dastor.thrift.server.ThriftGlue.*;
...
import com.bigdata.dastor.db.KeyspaceNotDefinedException;
...
import com.bigdata.dastor.thrift.Deletion;
...
public class ThriftValidation
{
    ...
    // !BIGDATA: validate the user-defined bucket name
    public static String validateUdBucketName(String tableName, String udBucketName, boolean isInsert) throws InvalidRequestException
    {
        KSMetaData ksMata = DatabaseDescriptor.getKSMetaData(tableName);
        if ((ksMata == null) || (ksMata.bucketMapper == null))
        {
            // do nothing, udBucketName == cfName
            return udBucketName;
        }
        
        String cfName = ksMata.bucketMapper.getBucketName(udBucketName);
        if (cfName == null)
        {
            throw new InvalidRequestException("The user-defined bucket " + udBucketName
                        + " cannot be mapped to a internal bucket.");
        }
        
        // TODO: we may do some udBucketName validation in the future?
        return cfName;
    }
}
