...
import com.bigdata.dastor.service.StorageServiceMBean;
import com.bigdata.dastor.streaming.StreamingService;
import com.bigdata.dastor.streaming.StreamingServiceMBean;

/**
 * JMX client operations for Dastor.
 */
public class NodeProbe
{
    private static final String fmtUrl = "service:jmx:rmi:///jndi/rmi://%s:%d/jmxrmi";
    private static final String ssObjName = "com.bigdata.dastor.service:type=StorageService";
    private static final int defaultPort = 8080;
    private String host;
    private int port;

    private JMXConnector jmxc; // BIGDATA
    ...
    private StorageProxyMBean spProxy; // BIGDATA
    ...
    private void connect() throws IOException
    {
        ...
        jmxc = JMXConnectorFactory.connect(jmxUrl, null); // BIGDATA
        ...
        try
        {
            ...
            // BIGDATA
            name = new ObjectName(StorageProxy.MBEAN_OBJECT_NAME);
            spProxy = JMX.newMBeanProxy(mbeanServerConn, name, StorageProxyMBean.class);
            name = new ObjectName(HintedHandOffManager.MBEAN_OBJECT_NAME);
    ...
    // BIGDATA: use to close jmx connection
    public void close()
    {
        try
        {
    ...
    // BIGDATA:
    public Iterator<Map.Entry<String, ColumnFamilyStoreMBean>> getColumnFamilyStoreMBeanProxies(String tableName)
    {
        try
        {
            return new ColumnFamilyStoreMBeanIterator(mbeanServerConn, tableName);
        }
        catch (MalformedObjectNameException e)
        {
            throw new RuntimeException("Invalid ObjectName? Please report this as a bug.", e);
    ...
    // BIGDATA:
    public ColumnFamilyStoreMBean getColumnFamilyMBean(String tableName, String cfName) throws MalformedObjectNameException, NullPointerException
    {
        String mbeanName = "com.bigdata.dastor.db:type=ColumnFamilyStores,keyspace=" + tableName + ",columnfamily=" + cfName;
        ColumnFamilyStoreMBean cfsProxy = JMX.newMBeanProxy(mbeanServerConn, new ObjectName(mbeanName), ColumnFamilyStoreMBean.class);
        return cfsProxy;
    }
    
    public JMXInstrumentedCacheMBean getKeyCacheMBean(String tableName, String cfName)
    {
        String keyCachePath = "com.bigdata.dastor.db:type=Caches,keyspace=" + tableName + ",cache=" + cfName + "KeyCache";
        try
        {
    ...
    {
        String rowCachePath = "com.bigdata.dastor.db:type=Caches,keyspace=" + tableName + ",cache=" + cfName + "RowCache";
        try
        {
    ...
    // BIGDATA:
    public StorageServiceMBean getStorageServiceMBean()
    {
        return ssProxy;
    }
    
    // BIGDATA:
    public StorageProxyMBean getStorageProxyMBean()
    {
        return spProxy;
    }
    
    // BIGDATA:
    public CompactionManagerMBean getCmMBean()
    {
        return mcmProxy;
    }
    
    // BIGDATA:
    public List<KSMetaData> getSchema() throws IOException 
    {
        List<KSMetaData> list = new ArrayList<KSMetaData>();
        for (byte[] b : ssProxy.getSchema())
        {
            ByteArrayInputStream input = new ByteArrayInputStream(b);
            list.add(KSMetaData.deserialize(input));
            input.close();
        }
        return list;
    }
    ...
    // BIGDATA:
    public Map<String, FileUtils.FSInfo> getStorageFSInfo() throws IOException, InterruptedException
    {
        return ssProxy.getStorageFSInfo();
    }
    
    // BIGDATA:
    public Map<String, FileUtils.FSInfo> getLogFSInfo() throws IOException, InterruptedException
    {
        return ssProxy.getLogFSInfo();
    }
    
    // BIGDATA:
    public HintedHandOffManagerMBean getHhProxyMBean()
    {
        return hhProxy;
    }
}
...
class ColumnFamilyStoreMBeanIterator implements Iterator<Map.Entry<String, ColumnFamilyStoreMBean>>
{
    ...
    {
        ObjectName query = new ObjectName("com.bigdata.dastor.db:type=ColumnFamilyStores,*");
        resIter = mbeanServerConn.queryNames(query, null).iterator();
        this.mbeanServerConn = mbeanServerConn;
    }
    
    // BIGDATA:
    public ColumnFamilyStoreMBeanIterator(MBeanServerConnection mbeanServerConn, String tableName)
    throws MalformedObjectNameException, NullPointerException, IOException
    {
        ObjectName query = new ObjectName("com.bigdata.dastor.db:type=ColumnFamilyStores,keyspace=" + tableName + ",*");
        resIter = mbeanServerConn.queryNames(query, null).iterator();
        this.mbeanServerConn = mbeanServerConn;
    }
...
class ThreadPoolProxyMBeanIterator implements Iterator<Map.Entry<String, IExecutorMBean>>
{
    ...
    throws MalformedObjectNameException, NullPointerException, IOException
    {
        ObjectName query = new ObjectName("com.bigdata.dastor.concurrent:type=*");
        resIter = mbeanServerConn.queryNames(query, null).iterator();
        this.mbeanServerConn = mbeanServerConn;
    }
